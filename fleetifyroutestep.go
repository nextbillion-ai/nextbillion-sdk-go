...
// Update a step
func (r *FleetifyRouteStepService) Put(ctx context.Context, stepsID string, params FleetifyRouteStepPutParams, opts ...option.RequestOption) (res *FleetifyRouteStepPutResponse, err error) {
	opts = append(r.Options[:], opts...)
	if params.RouteID == "" {
		err = errors.New("missing required routeID parameter")
		return
...
type FleetifyRouteStepPutResponse struct {
	Data RouteStepsResponse `json:"data"`
	// Returns the error message in case of a failed request. If the request is
	// successful, this field is not present in the response.
	Message string `json:"message"`
...
// Returns the unmodified JSON received from the API
func (r FleetifyRouteStepPutResponse) RawJSON() string { return r.JSON.raw }
func (r *FleetifyRouteStepPutResponse) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
...
type FleetifyRouteStepPutParams struct {
	RouteID string `path:"routeID,required" json:"-"`
	// A key is a unique identifier that is required to authenticate a request to the
	// API.
	Key string `query:"key,required" json:"-"`
	...
	// Driver's app under step details. This attribute can be used to provide context
	// about or instructions to the driver for performing the task
	Meta FleetifyRouteStepPutParamsMeta `json:"meta,omitzero"`
	// Specify the step type. It can belong to one of the following: `start`, `job` ,
	// `pickup`, `delivery`, `end`. A `duration` is mandatory when the step type is
	// either `layover` or a `break`.
	//
	// Any of "`start`", "`job`", "`pickup`", "`delivery`", "`break`", "`layover`",
	// "`end`".
	Type FleetifyRouteStepPutParamsType `json:"type,omitzero"`
	paramObj
}

func (r FleetifyRouteStepPutParams) MarshalJSON() (data []byte, err error) {
	type shadow FleetifyRouteStepPutParams
	return param.MarshalObject(r, (*shadow)(&r))
}
func (r *FleetifyRouteStepPutParams) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}

// URLQuery serializes [FleetifyRouteStepPutParams]'s query parameters as
// `url.Values`.
func (r FleetifyRouteStepPutParams) URLQuery() (v url.Values, err error) {
	return apiquery.MarshalWithSettings(r, apiquery.QuerySettings{
		ArrayFormat:  apiquery.ArrayQueryFormatComma,
		NestedFormat: apiquery.NestedQueryFormatBrackets,
	})
...
// Driver's app under step details. This attribute can be used to provide context
// about or instructions to the driver for performing the task
type FleetifyRouteStepPutParamsMeta struct {
	// Specify the name of the customer for which the step has to be performed.
	CustomerName param.Opt[string] `json:"customer_name,omitzero"`
	// Specify the phone number of the person to be contacted when at step location.
	CustomerPhoneNumber param.Opt[string] `json:"customer_phone_number,omitzero"`
...
func (r FleetifyRouteStepPutParamsMeta) MarshalJSON() (data []byte, err error) {
	type shadow FleetifyRouteStepPutParamsMeta
	return param.MarshalObject(r, (*shadow)(&r))
}
func (r *FleetifyRouteStepPutParamsMeta) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}
...
// `pickup`, `delivery`, `end`. A `duration` is mandatory when the step type is
// either `layover` or a `break`.
type FleetifyRouteStepPutParamsType string

const (
	FleetifyRouteStepPutParamsTypeStart    FleetifyRouteStepPutParamsType = "`start`"
	FleetifyRouteStepPutParamsTypeJob      FleetifyRouteStepPutParamsType = "`job`"
	FleetifyRouteStepPutParamsTypePickup   FleetifyRouteStepPutParamsType = "`pickup`"
	FleetifyRouteStepPutParamsTypeDelivery FleetifyRouteStepPutParamsType = "`delivery`"
	FleetifyRouteStepPutParamsTypeBreak    FleetifyRouteStepPutParamsType = "`break`"
	FleetifyRouteStepPutParamsTypeLayover  FleetifyRouteStepPutParamsType = "`layover`"
	FleetifyRouteStepPutParamsTypeEnd      FleetifyRouteStepPutParamsType = "`end`"
)
