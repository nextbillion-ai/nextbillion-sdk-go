...
func TestSkynetAssetLocationGetListWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Skynet.Asset.Location.GetList(
		context.TODO(),
		"id",
		nextbillionsdk.SkynetAssetLocationGetListParams{
			Key:          "key=API_KEY",
			Cluster:      nextbillionsdk.SkynetAssetLocationGetListParamsClusterAmerica,
			Correction:   nextbillionsdk.String("correction=mapmatch=1,interpolate=0,mode=`car`"),
			EndTime:      nextbillionsdk.Int(0),
			GeometryType: nextbillionsdk.SkynetAssetLocationGetListParamsGeometryTypePolyline,
